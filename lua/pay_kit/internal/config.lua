...
function M.configure(opts)
  ...
  -- `expires_in = false` is the explicit development opt-out: challenges are
  -- then issued with no expiry. Any non-positive number is rejected so `0` is
  -- not silently treated as "never expires".
